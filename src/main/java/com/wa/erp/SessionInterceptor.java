...
public class SessionInterceptor implements HandlerInterceptor{
	...
	public boolean preHandle(
		...
		// Allow login and public endpoints even if interceptor mapping is misconfigured
		String uri = request.getRequestURI();
		String ctx = request.getContextPath();
		String path = (ctx == null || ctx.isEmpty()) ? uri : uri.substring(ctx.length());
		if (path.contains("/loginForm.do") || path.contains("/loginProc.do")
				|| path.startsWith("/js/") || path.startsWith("/style/") || path.startsWith("/images/")) {
			return true;
		}
		//---------------------------------------------
		// HttpSession 객체 얻기
		...
		if(mid==null) {
			...
			//false 값을 리턴하면 @RequestMapping 이 붙은 메소드를 호출하지 않는다.
			return false;
		}
		else {
			//true 값을 리턴하면 @RequestMapping 이 붙은 메소드를 호출한다.
			return true;
		}
