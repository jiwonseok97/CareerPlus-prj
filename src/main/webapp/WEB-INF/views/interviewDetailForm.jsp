...
<%@ include file="boardSideCategori.jsp" %>
    ...
    <div id="wrap" class="container"  >
      <h1 style="text-align: center;">면접 게시판 상세페이지</h1>
      <form action="submit.php" method="POST" name="commentRegForm" class="commentRegForm">
          ...
          <center>
         <span onClick="pushboardname('interviewboard','interview')">[목록으로]</span>
         <c:if test="${sessionScope.p_no==boardDTO.p_no}">
          <input type="button" value="수정/삭제"
							onclick="goUpDelForm(${boardDTO.b_no},'interview','interviewboard');">
		</c:if>
     </center>
      ...
      <table style="margin: 0 auto;">
      	...
      	댓글
              ...
              <th  style="width: 10%; text-align: center; cursor:pointer; "onCLick= "goBoardDetailForm(${boardDTO.b_no},'interview', 'interviewboard','interviewboard','rec_count desc');">좋아요</th>
              </c:if>
             
             <c:if test="${param.comment_sort=='rec_count desc'}">
              <th  style="width: 10%; text-align: center; cursor:pointer; "onCLick= "goBoardDetailForm(${boardDTO.b_no},'interview', 'interviewboard','interviewboard','rec_count asc');">좋아요▼</th>
              </c:if>
             
             <c:if test="${param.comment_sort=='rec_count asc'}">
              <th  style="width: 10%; text-align: center; cursor:pointer; "onCLick= "goBoardDetailForm(${boardDTO.b_no},'interview', 'interviewboard','interviewboard','');">좋아요▲</th>
              </c:if>
      	</tr>
